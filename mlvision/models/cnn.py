...
class SimpleConvBlock(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, pooling_layer):
        ...
        self.conv = nn.Sequential(
            ...
            pooling_layer((2, 2)) if pooling_layer != nn.Identity() else pooling_layer(),
        )
...
class MLP(nn.Module):
    def __init__(self, input_features, linears, num_classes):
        ...
        for i, features in enumerate(linears):
            layers.append(linear_block(linears[i-1] if i > 0 else input_features, features))
...
class SimpleCnn(nn.Module):
    def __init__(self, num_classes, convolutions, pooling, last_pool, linears):
        ...
        convs.append(SimpleConvBlock(3, convolutions[0][0], kernel_size=convolutions[0][1], pooling_layer=pooling_layer))

        for i, (channels, kernel) in enumerate(convolutions[1:-1]):
            convs.append(SimpleConvBlock(convolutions[i][0], channels, kernel, pooling_layer))

        convs.append(SimpleConvBlock(convolutions[-2][0], convolutions[-1][0], kernel_size=convolutions[-1][1], pooling_layer=pooling_layer if last_pool else nn.Identity))
